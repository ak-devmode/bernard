...
## In-App Broadcast (Chatwoot UI)

**Added March 2026.** Broadcasts can now be sent directly from the Chatwoot web interface instead of SSH + CLI. The in-app system uses `Whatsapp::BroadcastFromSheetService` (Ruby) and stores structured audit logs in the database.

### How It Works

1. User selects a WhatsApp inbox and approved template in the Chatwoot Broadcasts UI
2. User connects a Google Sheet and maps columns to template variables
3. System reads the sheet, normalizes phone numbers, applies filters, and sends via Meta API
4. Results are shown in real-time in the Broadcast Detail panel

### Audit Trail — `broadcast_logs` Table

Every broadcast writes structured events to the `broadcast_logs` table for debugging without grepping server logs.
...
| phone | string | Recipient phone (when applicable) |
| level | string | `info`, `warn`, or `error` |
| message | text | Human-readable description |
| metadata | jsonb | Structured data (template info, error details, etc.) |
| created_at | datetime | Timestamp |

**Event types:**
| Event | Level | When |
|-------|-------|------|
| `started` | info | Broadcast job begins |
| `sheet_read` | info | Google Sheet rows loaded |
| `filter_warning` | warn | Filter column not found in sheet headers |
| `recipients_ready` | info | Recipients normalized, count finalized |
| `recipient_skipped` | warn | Row skipped (missing phone/name or invalid number) |
| `template_validated` | info | Template found and approved in synced templates |
| `pre_send_validated` | info | First recipient test build succeeded |
| `recipient_sent` | info | Message sent successfully to one recipient |
| `recipient_failed` | error | Message failed for one recipient (with Meta error code) |
| `contact_synced` | info | Chatwoot contact created or updated |
| `contact_sync_failed` | warn | Contact sync failed (non-fatal) |
| `conversation_synced` | info | Outbound message logged in conversation |
| `conversation_sync_failed` | warn | Conversation sync failed (non-fatal) |
| `completed` | info | Broadcast finished (includes summary stats) |
| `error` | error | Top-level job failure |

### Querying Broadcast Logs
...
Optional query params: `?level=error` or `?event=recipient_failed`

**Rails console (on server):**
```ruby
# All logs for a broadcast
BroadcastLog.where(broadcast_id: 123).order(:created_at)

# Just failures
BroadcastLog.where(broadcast_id: 123, level: 'error')

# Failures for a specific phone
BroadcastLog.where(broadcast_id: 123, phone: '62812345678', level: 'error')

# Recent broadcasts with systemic failures (>90% fail rate)
Broadcast.where(status: :completed).where('failed_count > 0').order(created_at: :desc).limit(10)
```

### Phone Number Handling

Phone numbers from Google Sheets are normalized before sending:
- `081234567890` → `6281234567890` (Indonesian local → international)
- `8123456789` → `628123456789` (Indonesian short → international)
- `61440121417` → `61440121417` (foreign numbers pass through as-is)
- Numbers are displayed in E.164 format in the UI: `+62 812 345 678`

Non-Indonesian numbers are NOT rejected — they are sent to Meta and Meta validates them. If a number is invalid, Meta returns an error which is recorded in the broadcast detail.

### Error Messages

All error messages are in English with Meta error codes in brackets for debugging. Example: `[Meta 132000] Template parameter count mismatch — check variable mapping matches template.`

### Template Variable Fallbacks

If a template variable (e.g., `day_part`) is missing from the Google Sheet, the system checks `config/broadcast_templates.json` for a fallback value. If no fallback is configured, the variable is sent as an empty string to Meta.

`broadcast_templates.json` is a static config file — it is NOT editable from the Chatwoot UI. To add or modify fallback values, edit the file directly on the server.
...
**Major Features in v4.0:**
- ✅ In-app broadcast via Chatwoot UI (no more SSH + CLI for daily use)
- ✅ Structured audit trail (`broadcast_logs` table) — queryable via API and Rails console
- ✅ 29 unique error paths with distinct messages and Meta error codes
- ✅ Pre-send validation (template check + first-recipient test build)
- ✅ E.164 phone number display format in UI
- ✅ Template variable fallbacks from `broadcast_templates.json`
- ✅ Non-Indonesian phone numbers accepted (Meta validates)
- ✅ Completion summary with fail rate and systemic failure detection
